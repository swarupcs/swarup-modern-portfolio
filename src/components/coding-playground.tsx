...
import { Play, RotateCw, Download, TerminalSquare, Code2, LayoutGrid } from "lucide-react"
...
type ConsoleMessage = { type: "log" | "warn" | "error" | "info"; text: string; ts: number }
...
function buildSrcDoc(html: string, css: string, js: string, useTailwindCDN: boolean) {
  const consoleBridge = `
    (function(){
  ...
  `
  const tailwindTag = useTailwindCDN ? `<script src="https://cdn.tailwindcss.com"></script>` : ""
...
export default function CodingPlayground() {
  ...
  useEffect(() => {
    const onMessage = (e: MessageEvent) => {
      ...
      const { type, payload } = e.data as { type: ConsoleMessage["type"]; payload: string[] }
    ...
    <section id="playground" className="py-16 md:py-24">
      <div className="container px-4 md:px-6">
        <div className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              Edit HTML, CSS, and JavaScript. Toggle auto-run, switch templates, and view logs in the console panel.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:flex sm:items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <Switch id="tailwind" checked={useTailwindCDN} onCheckedChange={setUseTailwindCDN} />
            </div>
            ...
            <div className="col-span-2 flex items-center gap-2 sm:col-span-1">
              ...
              <Button variant="outline" size="sm" onClick={() => resetToTemplate(template)}>
            ...
            <Card className="border">
              <CardHeader className="pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-base font-semibold tracking-tight">Console</CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                <div className="h-[140px] overflow-auto rounded-md border bg-muted/30 p-3 text-sm">
                  {consoleMessages.length === 0 ? (
                    <div className="text-muted-foreground">No output yet. console.log will appear here.</div>
                  ) : (
                    <ul className="space-y-1">
                      {consoleMessages.map((m, i) => (
                        <li key={m.ts + ":" + i} className={consoleColor[m.type]}>
        ...
        <p className="mt-6 text-xs text-muted-foreground">
          Tip: Use the Tailwind CDN toggle to quickly style your HTML without writing CSS.
        </p>
      </div>
    </section>
  )
