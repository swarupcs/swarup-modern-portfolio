...
import { Play, RotateCw, Download, TerminalSquare, Code2, Loader2 } from "lucide-react"
...
const LANGUAGE_MAP: Record<LanguageKey, { pistonLang: string; version: string; sample: string; filename?: string }> = {
  javascript: {
...
export default function MultiLanguageRunner() {
  ...
  const [filename, setFilename] = useState<string>(LANGUAGE_MAP["javascript"].filename || "main.txt")

  const langOptions = useMemo(() => Object.keys(LANGUAGE_MAP) as LanguageKey[], [])
      ...
      <div className="container px-4 md:px-6">
        <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-bold tracking-tight md:text-3xl">Run code in multiple languages</h2>
            <CardDescription className="mt-1 max-w-2xl">
              Supports JavaScript (Node), C, C++, Java, and Go. Provide optional standard input, run the code safely in
              a sandbox, and view stdout/stderr.
            </CardDescription>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <Select value={language} onValueChange={(v) => onLanguageChange(v as LanguageKey)}>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card className="border">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-semibold tracking-tight">Source Code</CardTitle>
            </CardHeader>
            <CardContent>
              <Textarea
            ...
            <CardContent>
              {error ? (
                ...
                <div className="space-y-4">
                  <div>
                    <div className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">stdout</div>
                    <pre className="whitespace-pre-wrap rounded-md border bg-muted/30 p-3 text-sm">
                      {output.run?.stdout || "(empty)"}
                    </pre>
                  </div>
                  <div>
                    <div className="mb-1 text-xs font-medium uppercase tracking-wide text-muted-foreground">stderr</div>
                    <pre className="whitespace-pre-wrap rounded-md border bg-muted/30 p-3 text-sm">
                  ...
                  {"compile" in output && output.compile ? (
                    <div>
                      ...
                      <pre className="whitespace-pre-wrap rounded-md border bg-muted/30 p-3 text-sm">
                        {output.compile?.stdout || output.compile?.stderr || "(empty)"}
                      </pre>
                    </div>
                  ) : null}
                  <div className="text-xs text-muted-foreground">
                    Exit code: {output.run?.code} {output.run?.signal ? `(signal: ${output.run.signal})` : ""}
                  </div>
                </div>
              ) : (
                <div className="text-sm text-muted-foreground">Run your code to see output here.</div>
              )}
            </CardContent>
          </Card>
        </div>
